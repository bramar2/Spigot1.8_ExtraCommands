...
public abstract class EnchantCommand {
    ...
    public EnchantCommand(String str) {
        if(hasArguments()) eachLine = str.split(":");
        else eachLine = new String[] {str};
        eachLine[0] = eachLine[0].replace(" ", "");
        if(!(eachLine[0].equalsIgnoreCase(name()) || (allowDamaged() && eachLine[0].equalsIgnoreCase("DAMAGED_"+name())))) throw new IllegalArgumentException("wrong id");
        ...
        if((index = needsIndex(EntityDamaged.class)) != -1) {
    ...
    private int needsIndex(Class<?> clazz) {
        ...
        for(Class<?> clazz2 : needed) {
            ...
            if(clazz2 == clazz || clazz2.isInstance(clazz) || instanceOf(clazz2, clazz, true)) return count;
            count++;
        }
        return -1;
    }
    // Should override this to return false for Commands that don't have arguments
    ...
    public final boolean needs(Class<?> clazz) {
        ...
        for(Object obj : inputted) {
            if(clazz.isInstance(obj) || instanceOf(obj, clazz, false)) return false; // Already inputted
        }
        for(Class<?> clazz2 : needed) {
            if(clazz.isInstance(clazz2) || instanceOf(clazz2, clazz, true)) return true;
        }
